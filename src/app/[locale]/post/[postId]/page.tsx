...
import type { PublishedPost, Comment, Mention, Locale } from '@/types';
...
const renderWithMentions = (text: string, mentions: Mention[] = [], locale: Locale) => {
        ...
        <p className="text-sm mt-1 whitespace-pre-wrap">
            {parts.map((part, i) => {
                ...
                if (i % 2 === 1 && mentionMap.has(mentionKey)) {
                    return (
                        <Link key={i} href={`/${locale}/profile/${mentionMap.get(mentionKey)}`} className="font-semibold text-primary hover:underline">
                            {mentionKey}
                        </Link>
                    );
                }
...
const CommentItem = ({ 
    ...
    return (
        <div className="flex items-start gap-4">
            <Link href={`/${locale}/profile/${comment.userId}`}>
            ...
            <div className="flex-grow">
                <div className="bg-muted p-3 rounded-lg">
                    <div className="flex items-center justify-between">
                        <Link href={`/${locale}/profile/${comment.userId}`} className="font-bold hover:underline">{comment.userName}</Link>
                        <p className="text-xs text-muted-foreground">
                            {formatDistanceToNow(new Date(comment.createdAt), { locale: es, addSuffix: true })}
                        </p>
                    </div>
                     {renderWithMentions(comment.text, comment.mentions, locale)}
                </div>
...
const PostContentRenderer = ({ content, mentions, locale }: { content: string, mentions?: Mention[], locale: Locale }) => {
    if (!mentions || mentions.length === 0) {
...
export default function PostDetailPage() {
    ...
    useEffect(() => {
        if (searchParams.get('tip_success')) {
            ...
            // Clean up the URL
            router.replace(`/${locale}/post/${params.postId}`, undefined);
        }
    }, [searchParams, toast, router, params.postId, locale]);
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-8">
            ...
            <Card className="shadow-xl">
                <CardHeader>
                    <div className="flex items-start gap-4">
                        <Link href={`/${locale}/profile/${post.publisherId}`}>
                        ...
                        <div className="flex-grow">
                            <Link href={`/${locale}/profile/${post.publisherId}`} className="font-semibold cursor-pointer hover:underline text-lg">{post.publisherName}</Link>
                        ...
                        {isOwner && (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <DropdownMenuItem onClick={() => router.push(`/${locale}/edit-post/${post.id}`)}>
                ...
                <CardContent className="space-y-6">
                    ...
                    {post.type === 'text' ? (
                        <PostContentRenderer content={post.content} mentions={post.mentions} locale={locale} />
                    ) : (
            ...
            <div className="space-y-6">
                ...
                {user ? (
                    ...
                    <p className="text-muted-foreground text-center">
                        <Link href={`/${locale}/login`} className="underline font-semibold">Inicia sesión</Link> para dejar un comentario.
                    </p>
                )}
                        ...
                        <AlertDialogDescription>
                            ...
                            <div className="flex items-center space-x-2 mt-4">
                                ...
                                <Label htmlFor="tip-agreement" className="text-sm font-normal text-muted-foreground">
                                    Entiendo y acepto las <Link href={`/${locale}/policies#monetization-policy`} className="underline" target="_blank">políticas de monetización</Link>.
                                </Label>
                            </div>
                        </AlertDialogDescription>
                    </AlertDialogHeader>
